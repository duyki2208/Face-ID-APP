...
class _HomePageState extends State<RecognitionScreen> {
  ...
  doFaceDetection() async {
    ...
    for (Face face in faces) {
      ...
      num right = boundingBox.right>image.width?image.width:boundingBox.right;
      num bottom = boundingBox.bottom>image.height?image.height:boundingBox.bottom;
...
class FacePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (Recognition face in facesList) {
      ...
      TextSpan textSpan = TextSpan(text: face.name,style: TextStyle(color: Colors.white,fontSize: 200));
